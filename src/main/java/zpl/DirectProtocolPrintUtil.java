 ...
 *@DESC
 */
public class DirectProtocolPrintUtil {
    ...
    public DirectProtocolPrintUtil(String ip) {
        this(ip, DEFAULT_PORT);
    }

    public DirectProtocolPrintUtil(String ip, int port) {
    ...
    public void printImage(int x, int y, BufferedImage image) {
        ...
        try {
            ...
            baos.write(sb.append("pp").append(x).append(",").append(y).append(":").append("PRBUF " + (data.length + 6)).append("\r\n").toString().getBytes());
    ...
    public void doPrint() throws IOException {
        ...
        OutputStream os = socket.getOutputStream();//new FileOutputStream("d:/doc/kkkkkkkkkkkkkkkkkkk.prn");//
